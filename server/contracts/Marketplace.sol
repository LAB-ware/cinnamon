...
contract Marketplace is ReentrancyGuard {
    ...
    // taken from the seller and transferred to the marketplace contract owner whenever an NFT is sold
    uint256 public LISTING_FEE = 0.0001 ether;

    // the Marketplace contract owner, so that we know who to pay the listing fee to
    ...
    // emitted every time a NFT is listed
    event NFTListed(
        address nftContract,
        uint256 tokenId,
        address seller,
    ...
    // emitted every time a NFT is sold
    event NFTSold(
        address nftContract,
        uint256 tokenId,
        address seller,
